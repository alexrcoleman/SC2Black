...
class CollectMineralShards(base_agent.BaseAgent):
  ...
  def step(self, obs):
    ...
    curUnit = self.cycle // 2
    if self.cycle == 0 or self.cycle == 2:
      return FUNCTIONS.select_army("select", curUnit)
    if self.cycle == 1 or self.cycle == 3:
      player_relative = obs.observation.feature_screen.player_relative
      selected = obs.observation.feature_screen.selected
      minerals = _xy_locs(player_relative == _PLAYER_NEUTRAL)
      if not minerals:
        return FUNCTIONS.no_op()
      marines = _xy_locs(selected == _PLAYER_SELF)
      marine_xy = marines[curUnit]#numpy.mean(marines, axis=0).round()  # Average location.
      #print(numpy.array(minerals))
      #print(marine_xy)
      #print(numpy.array(minerals) - marine_xy)
      distances = numpy.linalg.norm(numpy.array(minerals) - marine_xy, axis=1)
      #print(distances)
      #print("\n\n\n")
      indices = numpy.argpartition(distances, len(distances)-1)
      closest_mineral_xy = closest2_mineral_xy = minerals[indices[0]]
      if len(indices) > 1:
        closest2_mineral_xy = minerals[indices[len(indices)-1]]
      self.cycle = (self.cycle + 1) % 4
      return FUNCTIONS.Move_screen("now", closest2_mineral_xy)
